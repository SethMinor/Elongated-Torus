...
G = c.*L; % gamma evaluated over the field
% Dginv evaluated over the field
Dginv_matrix = reshape(Dginv(Theta_temp),[N,N]);
...
for i = 0:N_time
    ...
    tic;
    %k1 = RHS(psi, L, du, dv);
    k1 = RHS(psi, L, G, du, dv, Dginv_matrix, a, R, r, c, Phi_temp, Theta_temp);
    %k2 = RHS(psi + (dt/2)*k1, L, du, dv);
    k2 = RHS(psi  + (dt/2)*k1, L, G, du, dv, Dginv_matrix, a, R, r, c, Phi_temp, Theta_temp);
    %k3 = RHS(psi + (dt/2)*k2, L, du, dv);
    k3 = RHS(psi  + (dt/2)*k2, L, G, du, dv, Dginv_matrix, a, R, r, c, Phi_temp, Theta_temp);
    %k4 = RHS(psi + dt*k3, L, du, dv);
    k4 = RHS(psi  + dt*k3, L, G, du, dv, Dginv_matrix, a, R, r, c, Phi_temp, Theta_temp);
...
% Return the RHS
function F_of_psi = RHS(psi, L, G, du, dv, Dginv_matrix, a, R, r, c, Phi_temp, Theta_temp)
...
%     % Maybe try psi.^2 (?)
    ...
    % Updated isothermal Laplacian
    F_of_psi = (1i/2)*(-2*(1/(du^2) + 1/(dv^2))*psi ...
    + (1/(du^2))*( circshift(psi,1,2) + circshift(psi,-1,2) )...
    + (1/(dv^2))*( circshift(psi,1,1) + circshift(psi,-1,1) ))./(L.^2) ...
    + ( ... % Extra term .......
    ((R-a)*(a+R+2*r*cos(Theta_temp)).*sin(2*Phi_temp)./(2*c*(G.^3))).*((circshift(psi,1,2) + circshift(psi,-1,2))/(2*du)) ...
    + (r*(a+R+(R-a)*cos(2*Phi_temp)+2*r*cos(Phi_temp)).*sin(Theta_temp).*Dginv_matrix./(2*(G.^3))).*((circshift(psi,1,1) + circshift(psi,-1,1))/(2*dv)) ...
    )./(L) ... % end of extra term....
    - 1i*(conj(psi).*psi).*psi;
